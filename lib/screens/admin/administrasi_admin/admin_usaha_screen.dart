...
import 'package:essentials/services/update/update_ad_usaha.dart';
...
class Admin_UsahaScreen extends StatefulWidget {
  final String id;
  const Admin_UsahaScreen({super.key, required this.id});

  @override
  _Admin_UsahaScreenState createState() => _Admin_UsahaScreenState();
}

class _Admin_UsahaScreenState extends State<Admin_UsahaScreen> {
  final UploadUsahaService _UploadUsahaService = UploadUsahaService();
  File? selectedDocument;
  bool _isImageVisibleKTP = false;
  bool _isImageVisibleKK = false;
  late Future<Map<String, dynamic>?> _futureUsaha;
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, snapshot) {
          ...
          var usaha = snapshot.data!;
          String namaPemohon = usaha["nama"] ?? "Tidak diketahui";
          String noHpPemohon = usaha["no_hp"] ?? "Tidak diketahui";
          String emailPemohon = usaha["email"] ?? "Tidak diketahui";
          String tglUploadPemohon = usaha["us_tgl_upload"] != null
              ? DateFormat('dd MMMM yyyy - HH:mm:ss').format(
                  DateTime.parse(usaha["us_tgl_upload"]),
                )
              : "Tidak diketahui";
          String fotoKTP = usaha["us_foto_ktp"] ?? "Tidak diketahui";
          String fotoKK = usaha["us_foto_kk"] ?? "Tidak diketahui";
          String omsetBulan = usaha["us_omset"] ?? "Tidak diketahui";
          String JudulPengaduan = usaha["us_judul"] ?? "Tidak diketahui";
          String suratKonfirmasiData =
              usaha["us_surat_konfirmasi"] ?? "Tidak diketahui";
          String konfirmasiData = usaha["us_konfirmasi"] ?? "Tidak diketahui";
  ...
  Widget _document(
    ...
    return GestureDetector(
      onTap: () async {
        ...
        print("ID Usaha yang dikirim: ${widget.id}");
        if (widget.id.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("ID Usaha tidak valid"),
              backgroundColor: Colors.red,
            ),
          );
          return;
        ...
        String? filePath = await downloadServicesUsaha.downloadAdministrasiUsaha(
            context, widget.id);
