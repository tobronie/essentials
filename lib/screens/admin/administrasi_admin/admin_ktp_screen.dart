...
class _Admin_KTPScreenState extends State<Admin_KTPScreen> {
  ...
  Future<Map<String, dynamic>?> getKTP() async {
    String url =
        'https://essentials.my.id/get_ad_ktp.php?id_ktp=${widget.id}';
    try {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>?>(
        ...
        builder: (context, snapshot) {
          ...
          String JudulPengaduan = ktp["kt_judul"] ?? "Tidak diketahui";
          String suratKonfirmasiData =
              ktp["kt_surat_konfirmasi"] ?? "Tidak diketahui";
  ...
  Widget _document(
    ...
    return GestureDetector(
      onTap: () async {
        ...
        print("ID KTP yang dikirim: ${widget.id}");
        if (widget.id.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("ID KTP tidak valid"),
              backgroundColor: Colors.red,
            ),
          );
          return;
        ...
        String? filePath = await downloadServicesKTP.downloadAdministrasiKTP(
            context, widget.id);
